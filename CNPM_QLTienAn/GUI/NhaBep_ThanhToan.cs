...
namespace CNPM_QLTienAn.GUI
{
    public partial class NhaBep_ThanhToan : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void LoadDataIntoComboBoxLop(string tenDonVi, List<DonVi> lstDonVi)
        {
            ...
            var lstLopHocVien = db.HocViens.Where(s => s.MaDonVi == maC).Select(s => new { s.Lop }).Distinct().ToList();
        ...
        private List<Class_DuLieuCatComHienthiGirdView> LoadDataGridControl2()
        {
            ...
            {
                ThanhToan temp = db.ThanhToans.Where(s => s.MaThanhToan == item.maThanhToan).First();
            ...
            foreach (var item in lstChuaThanhToan)
            {
                DanhSachNghi temp = db.DanhSachNghis.Where(s => s.MaDS == item.MaDS).First();
