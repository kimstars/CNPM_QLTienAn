...
{
    public partial class Admin_HocVien : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void btnSuaHV_Click(object sender, EventArgs e)
        {
            ...
            if (txtSuaMaDVHV.Text != "" && txtSuaMaTCA.Text != "")
            {
                ...
                var temp_hv = db.DonVis.Where(p => p.MaDonVi == madvhv).FirstOrDefault();
                if (temp_hv == null)
                {
                ...
                var temp_hv1 = db.TieuChuanAns.Where(p => p.MaTCA == matcahv).FirstOrDefault();
                if (temp_hv1 == null)
                {
