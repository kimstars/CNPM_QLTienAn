    ...
    public partial class NhaBep_ThongKe : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void LoadDataChart1()
        {
            ...
            List<ThanhToan> lstThanhToan =
                db.ThanhToans.Where(s => DbFunctions.TruncateTime(s.NgayTT) >= dtpDau.DateTime.Date && DbFunctions.TruncateTime(s.NgayTT) <= dtpCuoi.DateTime.Date).ToList();
        ...
        {
            List<ChiTietNghi> lstCTT = db.ChiTietNghis.Where(s => DbFunctions.TruncateTime(s.NgayNghi) >= dtpDau.DateTime.Date && DbFunctions.TruncateTime(s.NgayNghi) <= dtpCuoi.DateTime.Date).ToList();
